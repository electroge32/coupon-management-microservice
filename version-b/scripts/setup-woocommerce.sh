...
# Configurar estructura de permalinks (requerida para WC REST API — sin esto devuelve HTML)
log "Configurando estructura de permalinks..."
wp option update permalink_structure "/%postname%/" --allow-root --path="$WP_PATH"
wp rewrite flush --allow-root --path="$WP_PATH"
log "Permalinks configurados (/%postname%/)."

# Asegurar que siteurl y home apunten a localhost:8081 (acceso desde browser host)
# WC_BASE_URL del servicio app sigue siendo http://wordpress (DNS interno Docker)
log "Configurando siteurl/home a http://localhost:8081..."
wp option update siteurl "http://localhost:8081" --allow-root --path="$WP_PATH"
wp option update home "http://localhost:8081" --allow-root --path="$WP_PATH"
log "siteurl y home actualizados."
